...
<?php 
	...
	/*$result = mysqli_query($conn,"SELECT title FROM image WHERE id = '$dd'");
	
	$sql = "select * from image where id='$dd' ";
    $result = $con->query($sql);   
	$row = mysqli_fetch_array($result);*/
	
	/*$db = mysqli_select_db("image", $conn);
	$query = mysql_query("select * from image where id=$dd", $conn);*/
 	...
 	<div class="modal-content">
 		...
 		<div class="modal-body">
 			...
 			<div class="form-group">
 				...
 				<input type="text" class="form-control" name="title" value="<?php echo $row1['title']; ?>" required>
 			</div>
 			<div class="form-group">
 				<label for="title" class="form-control-label p-0">Body</label>
 				<input type="text" class="form-control" name="body" value="<?php echo $row1['body']; ?>" required>
 			</div>
 			<div class="form-group">
 			...
 			<div class="form-group">
				...
				<input type="file" class="font-control-file" id="file" name="image" value="<?php echo $row1['image1'];?>" required >
 			</div>
...
if(isset($_POST['add'])) {
 ...
 move_uploaded_file($book_CoverTMP, $folder.$book_Cover);
		...
		{
			$sql = "INSERT INTO `image`(`category`, `title`, `image1`, `body`)VALUES('$category_name','$book_name','$book_Cover','$aname')";
			$qry = mysqli_query($conn, $sql);
			echo "<script>alter('Data updated successfully');</script>";
